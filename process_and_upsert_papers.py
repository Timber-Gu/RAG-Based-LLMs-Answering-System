...
def normalize_text_for_chunking(text: str) -> str:
    """Normalize text prior to chunking (fix hyphenation, collapse whitespace)."""
    cleaned = re.sub(r"-\s*\n\s*", "", text)
...
def create_paragraph_chunks(text: str, title: str, paper_id: str, 
    ...
    """
    # Normalize text first for more stable chunking
    text = normalize_text_for_chunking(text)
    
    # Extract sentences while preserving ending punctuation
    sentences = re.findall(r'[^.!?]+[.!?]+(?=\s|$)', text)
    if not sentences:
        sentences = [text]
    else:
        # Include trailing text without punctuation if present
        end_idx = sum(len(s) for s in sentences)
        if end_idx < len(text):
...
def process_existing_pdfs_with_chunking(force: bool = False):
    """
    ...
    # Track existing IDs; do not globally remove chunks here
    ...
    for pdf_file in pdf_files:
        ...
        # If not forced and already processed, skip paper
        if not force and any(e.get('parent_paper_id') == arxiv_id for e in knowledge_base):
            print(f"  ⏭️  Skipping {arxiv_id}: already present (use --force to reprocess)")
            continue
        ...
        try:
            ...
            to_remove_ids = {e['id'] for e in knowledge_base if e.get('parent_paper_id') == arxiv_id}
            knowledge_base = [e for e in knowledge_base if e.get('parent_paper_id') != arxiv_id]
            existing_ids -= to_remove_ids
            removed = before - len(knowledge_base)
            if removed:
                print(f"  🧹 Removed {removed} existing chunks for {arxiv_id}")
...
def main():
    ...
    if not args.upsert_only:
        ...
        success = process_existing_pdfs_with_chunking(force=args.force)
